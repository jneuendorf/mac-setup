...
# defaults = {
#     # (command_conf['command'], command_conf['label'])
#     f'{command_conf["command"].replace(" write ", " read ")}  # {command_conf["label"]}'
#     for category, commands in raw_command_config.items()
#     for command_conf in commands.values()
#     if 'command' in command_conf and command_conf['command'].startswith('defaults')
# }
...
def transformed_config(command_conf):
    ...
    # print(command)
    host = ''
    if '-currentHost' in command:
        host = '-currentHost'
        command = command.replace(host, '')

    domain, key, param_type, *rest = shlex.split(command)[2:]

    description = command_conf['label']

    if 'choices' in command_conf:
        choices = {tup[0] for tup in command_conf['choices']}
        description += ' Choices: ' + ', '.join(
            f'{choice[0]} ({choice[1]})'
            for choice in command_conf['choices']
        )
    else:
        choices = set()
    ...
    if rest:
        ...
        if len(rest) > 1:
            # print('SPECIAL CASE!')
            # print(rest)
            raise ValueError('Could not handle command')
        elif rest[0] != '{0}':
            choices.add(rest[0])
    ...
    return {
        ...
        **(
        ...
        'param': {
            'type': param_type[1:],  # remove leading dash
            **(
                {'values': list(choices)}
                if choices
                else {}
        ...
        'versions': [],
        **(
            {'after': commands['_meta']['run_after']}
            if '_meta' in commands and 'run_after' in commands['_meta']
            else {}
        ),
        **(
            {'sudo': True}
            if command_conf.get('sudo', False)
            else {}
        ),
    }
...
for category, commands in raw_command_config.items():
    keys = []
    for title, command_conf in commands.items():
        if title == '_meta':
            continue
        ...
        if 'command' in command_conf and command_conf['command'].startswith('defaults'):
            ...
            try:
                transformed = transformed_config(command_conf)
...
with open(Path('defaults2.json'), 'w') as file:
